...
void build_lcp(char *s,int *sa,int n){
	int i;
	...
	for(i=0,h=0;i<n;i++){
		if(rank[i]<n-1){
			for(j=sa[rank[i]+1];s[i+h]==s[j+h];++h){
				//printf("%d %d %d %d\n",j,sa[j],rank[j],h);
			}
			lcp[rank[i]]=h;
			if(h>0) --h;
...
int search(char *pat,int m, char *txt, int *suffArr, int n)
{
...
void solve(){
	...
	for(i=0;i<n;++i) a[i]=CONVERT(s[i])+1;
	int k=64,l=0;
	
	suffixsort(a,sa,n,k,l);
	/*
	build_lcp(s,sa,n);
	puts(s);
	for(i=0;i<n;++i){
